        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($article->suggested_tags && auth()->user()->can('publish-article'))
                <div class="mb-4 p-4 rounded-md bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-300">
                    ...
                    <p class="text-sm">Jurnalis menyarankan tag berikut. Jika Anda setuju, silakan buat tag baru (di tab
                        terpisah) lalu tambahkan ke artikel ini. Menyimpan artikel ini akan menghapus saran tersebut.
                    </p>
                    <div class="mt-2 flex flex-wrap gap-2">
                        @foreach ($article->suggested_tags as $suggestedTag)
                            <span
                                class="px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-200 dark:bg-yellow-800/50 text-yellow-900 dark:text-yellow-200">
                                {{ $suggestedTag }}
                            </span>
                        @endforeach
                    </div>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('admin.articles.update', $article) }}"
                        ...
                        <div class="mt-4" wire:ignore x-data="tomselect({
                            ...
                            <select id="tags" name="tags[]" multiple x-ref="tomselect" class="block mt-1 w-full"
                            ...
                            <div class="mt-4">
                                ...
                                <select name="status" id="status"
                                    ...
                                    <option value="{{ App\Models\Article::STATUS_DRAFT }}"
                                        {{ old('status', $article->status) == App\Models\Article::STATUS_DRAFT ? 'selected' : '' }}>
                                        Draft</option>
                                    <option value="{{ App\Models\Article::STATUS_PENDING }}"
                                        {{ old('status', $article->status) == App\Models\Article::STATUS_PENDING ? 'selected' : '' }}>
                                        Pending Review</option>
                                    <option value="{{ App\Models\Article::STATUS_PUBLISHED }}"
                                        {{ old('status', $article->status) == App\Models\Article::STATUS_PUBLISHED ? 'selected' : '' }}>
                                        Published</option>
                                </select>
                            </div>
                            <div class="mt-4 space-y-2">
                                <label for="is_hero_pinned" class="flex items-center">
                                    <input type="checkbox" id="is_hero_pinned" name="is_hero_pinned" value="1"
                                        @checked(old('is_hero_pinned', $article->is_hero_pinned))
                                        class="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600
                                text-brand-primary dark:text-brand-primary
                                focus:ring-brand-accent dark:focus:ring-brand-accent">
                                    <span
                                        class="ms-2 text-sm text-gray-700 dark:text-gray-300">{{ __('Jadikan Hero Utama Homepage') }}</span>
                                </label>
                                <label for="is_editors_pick" class="flex items-center">
                                    <input type="checkbox" id="is_editors_pick" name="is_editors_pick" value="1"
                                        @checked(old('is_editors_pick', $article->is_editors_pick))
                                        class="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600
                                text-brand-primary dark:text-brand-primary
                                focus:ring-brand-accent dark:focus:ring-brand-accent">
                                    <span
                                        class="ms-2 text-sm text-gray-700 dark:text-gray-300">{{ __('Jadikan Pilihan Editor (Slider)') }}</span>
                                </label>
                            </div>
                        @endcan
